...
const HOST_1 = {
  email: "pedromcm1623@gmail.com",
  name: "Pedro16"
};

const HOST_2 = {
  email: "pedromcm1723@gmail.com",
  name: "Pedro17"
};
...
const NEW_PROPERTIES = [
  ...
  },
  {
    ...
    hostName: HOST_2.name,
    hostEmail: HOST_2.email,
...
async function main() {
  console.log("Seeding properties for Pedro16 and Pedro17...");
  ...
  for (const p of NEW_PROPERTIES) {
    ...
    if (!user) {
      ...
      user = await prisma.user.update({
        where: { id: user.id },
        data: { role: "HOST", name: p.hostName }
      });
    }
    ...
    if (existing) {
      console.log(`Property ${p.name} already exists. Updating it.`);

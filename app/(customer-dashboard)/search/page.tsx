  ...
  return (
    <div className="min-h-screen">
      ...
      {view === "grid" ? (
        ...
        <MapView listings={filtered} onOpen={setOpenListing} />
      )}
...
function MapView({ listings, onOpen }: { listings: Listing[]; onOpen: (l: Listing) => void }) {
  return (
    <section className="mx-auto max-w-6xl px-4 pb-16">
      <div className="relative w-full aspect-[16/10] rounded-2xl border-2 border-foreground shadow-hard-lg overflow-hidden bg-gradient-to-br from-ocean/20 via-cream to-mustard/30">
        ...
        <div
          ...
          style={{
            backgroundImage:
              "linear-gradient(oklch(0.38 0.12 250 / 0.4) 1px, transparent 1px), linear-gradient(90deg, oklch(0.38 0.12 250 / 0.4) 1px, transparent 1px)",
        ...
        <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 62" preserveAspectRatio="none">
          <path
            d="M0,20 Q15,10 30,22 T60,25 T100,18 L100,62 L0,62 Z"
            fill="oklch(0.83 0.16 88 / 0.35)"
            stroke="oklch(0.19 0.02 60)"
            strokeWidth="0.3"
          />
        </svg>
        ...
        {listings.map((l) => (
          ...
          >
            <div className="relative flex flex-col items-center">
              <div className="bg-primary text-primary-foreground border-2 border-foreground shadow-hard rounded-full px-3 py-1 text-xs font-bold whitespace-nowrap group-hover:-translate-y-0.5 transition-transform">
                €{l.price}
              </div>
              <div className="w-3 h-3 bg-primary border-2 border-foreground rotate-45 -mt-1.5" />
              <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute top-full mt-2 bg-cream border-2 border-foreground shadow-hard-sm rounded-lg px-2 py-1 text-xs font-bold whitespace-nowrap">
                {l.name}
              </div>
            </div>
          </button>
